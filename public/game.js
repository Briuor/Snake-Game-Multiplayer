...
export default class Game {
	constructor() {
		...
		// init food
		this.food = new Food(Math.floor(Math.random() * this.canvas.width), Math.floor(Math.random() * this.canvas.height));

		this.mySnake = new Snake(this.socket.id, Math.floor(Math.random() * this.canvas.width), Math.floor(Math.random() * this.canvas.height), 32, 32, 32);
	...
	update = () => {
		...
		if(this.mySnake.checkCollisionMysnake()) {

		}

		// check collision with mySnake's body
		if(this.mySnake.checkCollisionSnakes(this.snakes)) {
			console.log('you lose');
		}


		// check if food was eated
		if(this.mySnake.checkCollisionFood(this.food, this.inputHandler.direction)) {
			this.food.generateFood(this.canvas);
			this.socket.emit('newfood', this.food);
		}

		this.mySnake.movePartsOfBody();
